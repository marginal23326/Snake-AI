...
impl GeneKey {
    fn read(self, cfg: &AiConfig) -> f64 {
        match self {
            Self::TrapDanger => cfg.scores.trap_danger,
            Self::StrategicSqueeze => cfg.scores.strategic_squeeze,
            Self::EnemyTrapped => cfg.scores.enemy_trapped,
            Self::HeadOnCollision => cfg.scores.head_on_collision,
            Self::TightSpot => cfg.scores.tight_spot,
            Self::Length => cfg.scores.length,
            Self::EatReward => cfg.scores.eat_reward,
            Self::TerritoryControl => cfg.scores.territory_control,
            Self::KillPressure => cfg.scores.kill_pressure,
            ...
            Self::Aggression => cfg.scores.aggression,
        }
    }

    fn write(self, cfg: &mut AiConfig, value: f64) {
        match self {
            Self::TrapDanger => cfg.scores.trap_danger = value,
            Self::StrategicSqueeze => cfg.scores.strategic_squeeze = value,
            Self::EnemyTrapped => cfg.scores.enemy_trapped = value,
            Self::HeadOnCollision => cfg.scores.head_on_collision = value,
            Self::TightSpot => cfg.scores.tight_spot = value,
            Self::Length => cfg.scores.length = value,
            Self::EatReward => cfg.scores.eat_reward = value,
            Self::TerritoryControl => cfg.scores.territory_control = value,
            Self::KillPressure => cfg.scores.kill_pressure = value,
            ...
            Self::Aggression => cfg.scores.aggression = value,
        }
    }
}
...
pub(super) fn crossover(a: &[f64], b: &[f64], rng: &mut StdRng) -> Vec<f64> {
    a.iter()
        .zip(b.iter())
        .map(|(x, y)| if rng.random::<f64>() < 0.5 { *x } else { *y })
        .collect()
}
